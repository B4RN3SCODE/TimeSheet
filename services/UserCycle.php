...
$period_ids = array();
foreach($data as $period_id => $period_data) $period_ids[] = $period_id;

$Clients = array();
foreach($period_ids as $data_key) {
	foreach($data[$data_key]["Client"] as $client_id => $client_data)
		$Clients[] = $client_data["Name"];
}
...
foreach($Clients as $tmp_client) {
	$Sheets[substr($tmp_client,0,31)] = $objPHPExcel->createSheet(++$sheet_count);
}
...
foreach($data as $periodId => $periodData) {
	...
	foreach($periodData["Client"] as $clientId => $clientData) {
		$ActiveSheetName = substr($clientData["Name"],0,31);
		SetActiveSheetByName($objPHPExcel, $ActiveSheetName);
		$col = 1; $row = 1;
		$Sheets[$ActiveSheetName]->setCellValueByColumnAndRow($col,$row,"Employee");
		$Sheets[$ActiveSheetName]->setCellValueByColumnAndRow(++$col,$row,$User->GetName());
		$col = 1;
		$Totals[$periodId][$clientId] = array();
		$colStartChar = $Sheets[$ActiveSheetName]->setCellValueByColumnAndRow($col,++$row,"Client",true)->getColumn();
		$colEndChar = $Sheets[$ActiveSheetName]->setCellValueByColumnAndRow(++$col,$row,$clientData["Name"],true)->getColumn();
		ApplyStyle($Sheets[$ActiveSheetName],$colStartChar,$colEndChar,$row,null,$styles["font-bold"]);
		foreach($clientData["Project"] as $projectId => $projectData) {
			...
			$col = 1;
			$row += 2;
			$Sheets[$ActiveSheetName]->setCellValueByColumnAndRow($col,$row,"Project",true);
			$Sheets[$ActiveSheetName]->setCellValueByColumnAndRow(++$col,$row,$projectData["Name"]);
			$col = 1;
			$colStartChar = $Sheets[$ActiveSheetName]->setCellValueByColumnAndRow($col,++$row,"Date",true)->getColumn();
			$table[$objPHPExcel->getActiveSheetIndex()]["start"][] = PHPExcel_Cell::stringFromColumnIndex($col) . $row;
			$Sheets[$ActiveSheetName]->setCellValueByColumnAndRow(++$col,$row,"Description");
			$Sheets[$ActiveSheetName]->setCellValueByColumnAndRow(++$col,$row,"Hours");
			$Sheets[$ActiveSheetName]->setCellValueByColumnAndRow(++$col,$row,"Travel");
			$colEndChar = $Sheets[$ActiveSheetName]->setCellValueByColumnAndRow(++$col,$row,"Billable",true)->getColumn();
			ApplyStyle($Sheets[$ActiveSheetName],$colStartChar,$colEndChar,$row,null,$styles["font-italic"]);
			foreach($projectData["Entry"] as $entryId => $entryData) {
				...
				$col = 1;
				$Sheets[$ActiveSheetName]->setCellValueByColumnAndRow($col,++$row,format_date($entryData['Date']));
				$Sheets[$ActiveSheetName]->setCellValueByColumnAndRow(++$col,$row,$entryData['Description']);
				$Sheets[$ActiveSheetName]->setCellValueByColumnAndRow(++$col,$row,$entryData['Hours']);
				$Sheets[$ActiveSheetName]->setCellValueByColumnAndRow(++$col,$row,$entryData['Travel']);
				$Sheets[$ActiveSheetName]->setCellValueByColumnAndRow(++$col,$row,($entryData['Billable'] == 1) ? "Yes" : "No");
			}
			$Sheets[$ActiveSheetName]->setCellValueByColumnAndRow(1,++$row,"Total");
			$Sheets[$ActiveSheetName]->setCellValueByColumnAndRow(3,$row,$Totals[$periodId][$clientId][$projectId]["Hours"]);
			$Sheets[$ActiveSheetName]->setCellValueByColumnAndRow(4,$row,$Totals[$periodId][$clientId][$projectId]["Travel"]);
			$Sheets[$ActiveSheetName]->mergeCellsByColumnAndRow(1, $row,2, $row);
			$Sheets[$ActiveSheetName]->getStyle("B$row:E$row")->applyFromArray($styles["font-bold"]);
